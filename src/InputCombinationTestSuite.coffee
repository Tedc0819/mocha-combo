class InputCombinationTestSuite
  ...
  runTest: (test, combination) ->
    ...
    self.stub test, combination
      .then ->

        self.setFixtures test, combination

      .then ->

        argValues = self.gatherArgsValue test, combination

        test.args = argValues
        self.testMethod test, combination, argValues
  ...
  run: () ->
    ...
    describe self.title(), ->
      ...
      combinations.forEach (combination) ->
        ...
        shouldSuccess = self.shouldSuccess combination

        descFunc =  ->

          before ->

            self.before @, combination

          beforeEach ->

            self.beforeEach @, combination

          after ->

            self.after @, combination

          afterEach ->

            self.afterEach @, combination

          if shouldSuccess

            self.successAssert combination

          else

            self.failureAssert combination

        if ("#{combination}" is "#{self.only()}")
        ...
        describe self.combinationTestTitle(shouldSuccess, combination), descFunc
  ...
  shouldSuccess: (combination) ->
    ...
    combination.forEach (inputType) ->

      count += 1 if inputType == 'correct'
  ...
  successAssert: (combination) ->
    ...
    it 'WARNING: Success Assert Not Implemented', ->
  ...
  failureAssert: (combination) ->
    ...
    it 'WARNING: Failuer Assert Not Implemented', ->
  ...
  title: -> @methodName + '(' + @args + ')'

  combinations: ->

    self = @

    extractedTypeArrays = @args.map (arg) -> self.argTypes[arg]

    @getCombinations(extractedTypeArrays).concat self.extraCombinations
  ...
  getCombinations: (array) ->
    ...
    combine = (array) ->
      ...
      lastResult = combine array.slice(1)

      array[0].forEach (first) ->
  ...
  combinationTestTitle: (shouldSuccess, combination) ->
    ...
    state = if shouldSuccess then 'success' else 'failure'

    tmp = combination.map (str) -> "'" + str + "'"

    combinationStr = combination.join ', '

    "#{state} - [#{combinationStr}]"
  ...
  gatherArgsValue: (test, combination) ->
    ...
    @args.map (arg, idx) ->
      self.getArgValues test, combination, arg, combination[idx]
